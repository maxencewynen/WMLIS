...
def get_train_transforms(I=['FLAIR'], apply_mask=None):
    ...
    transform_list = [
        ...
        #SaveImageKeysd(keys=I + masks, output_dir="/home/mwynen/data/cusl_wml/debug/", suffix="before"),
    ]
    if apply_mask:
    ...
    transform_list += [
        RandCropByPosNegLabeld(keys=I + ["instance_mask", "label"],
                               label_key="label", image_key=I[0], image_threshold=-0.1,
                               spatial_size=(128, 128, 128), num_samples=32,
                               pos=4, neg=1),
        RandSpatialCropd(keys=I + ["label", "instance_mask"],
                         roi_size=(96, 96, 96),
                         random_center=True, random_size=False),
        RandFlipd(keys=I + ["label", "instance_mask"], prob=0.5, spatial_axis=(0, 1, 2)),
        RandRotate90d(keys=I + ["label", "instance_mask"], prob=0.5, spatial_axes=(0, 1)),
        RandRotate90d(keys=I + ["label", "instance_mask"], prob=0.5, spatial_axes=(1, 2)),
        RandRotate90d(keys=I + ["label", "instance_mask"], prob=0.5, spatial_axes=(0, 2)),
        RandAffined(keys=I + ["label", "instance_mask"],
                    mode=('nearest', 'nearest', 'nearest'),#)tuple(['bilinear'] * (len(I) + 2)) + tuple(['nearest']),
